...
zeta = np.linspace(0, np.pi, 100, endpoint=True)
...
# data = eq.compute(["R", "Z", "zeta", "|B|"], grid=grid, jitable=True)
...
data = eq.compute(["R", "Z", "zeta", "|F|"], grid=grid)
# data = eq.compute(["R", "Z", "zeta", "|F|"], grid=grid, jitable=True)

mesh_end0 = [
    data["R"].reshape(shape),
    data["zeta"].reshape(shape),
    data["Z"].reshape(shape),
    data["|F|"].reshape(shape),
]
...
grid = Grid(
    nodes=np.stack(
        [
            rho_mesh.reshape(-1),
            theta_mesh.reshape(-1),
            np.pi * np.ones_like(rho_mesh.reshape(-1)),
        ]
    ).T,
    sort=False,
)
data = eq.compute(["R", "Z", "zeta", "p"], grid=grid)
# data = eq.compute(["R", "Z", "zeta", "p"], grid=grid, jitable=True)
